      ...
      <div className="mt-12 grid gap-8 md:grid-cols-2 lg:grid-cols-3">
        {posts.map(({ slug, title, date, summary, image, readingTime, tags }) => (
          <Link
            href={`/blog/${slug}`}
            key={slug}
            className="group flex flex-col overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm 
              ...
              <div className="mt-5 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-gray-500 dark:text-gray-400">
                ...
                {tags && tags.length > 0 && (
                   <div className="flex items-center flex-shrink min-w-0">
                     ...
                     <span className="truncate">{tags.slice(0, 2).join(', ')}{tags.length > 2 ? '...' : ''}</span>
                   </div>
                )}
              </div>
            </div>
